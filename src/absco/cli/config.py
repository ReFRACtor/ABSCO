...
def build_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        nargs="+",
        help="Output (degraded) resolution [cm-1]. One value applied to all bands, "
        "or one per band. Required (prompted if omitted).",
    )
    parser.add_argument(
        "--wn1",
        type=float,
        nargs="+",
        help="Band start wavenumber(s) [in --units]. Pair with --wn2.",
    )
    parser.add_argument(
        "--wn2",
        type=float,
        nargs="+",
        help="Band end wavenumber(s) [in --units]. Pair with --wn1.",
    )
    parser.add_argument(
        "--range",
        type=float,
        nargs=2,
        metavar=("WN1", "WN2"),
        help="Convenience for a single band: --range WN1 WN2.",
    )
    parser.add_argument(
        "--molnames",
        nargs="+",
    ...
    parser.add_argument(
        ...
        default="cm-1",
        choices=["cm-1", "um", "nm"],
        help="Spectral units for wn1/wn2.",
    )
    parser.add_argument(
        "--wv-vmr",
        type=float,
...
def main():
    ...
    lblres = ca.suggest_lblres(outres) if args.lblres is None else \
    ...
    ram = ca.estimate_ram_gb(wn1, wn2, outres, molnames)
    if ram is not None:
